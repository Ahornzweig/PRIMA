    ...
    class Control extends ƒ.Node {
        ...
        freeze() {
            ...
            for (let cube of this.fragment.getChildren()) {
                ...
                let element = new L09_FudgeCraft_CameraControl.GridElement(cube);
                //element.position = position;
                L09_FudgeCraft_CameraControl.grid.push(position, element);
                result.push(element);
            }
            return result;
        }
        detectCombo(_elements) {
            let combos = [];
            for (let element of _elements) {
                console.log(element.cube);
            }
            return combos;
        }
    }
